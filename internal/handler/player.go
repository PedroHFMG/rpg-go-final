...
func (ph *PlayerHandler) AddPlayer(w http.ResponseWriter, r *http.Request) {
	...
	result, err := ph.PlayerService.AddPlayer(player.Nickname, player.Life, player.Attack, player.Defesa, player.Status)
	if err != nil {
		switch {
...
func (ph *PlayerHandler) SavePlayer(w http.ResponseWriter, r *http.Request) {
	...
	result, err := ph.PlayerService.SavePlayer(id, player.Nickname, player.Life, player.Attack, player.Defesa, player.Status)
	if err != nil {
		switch {
		case strings.Contains(err.Error(), "internal server error"):
			w.WriteHeader(http.StatusInternalServerError)
